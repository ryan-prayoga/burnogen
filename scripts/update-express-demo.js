...
const fixtureRoot = path.join(repoRoot, "tests", "fixtures", "express");
const generatedRoot = path.join(fixtureRoot, ".brunogen");
const demoRoot = path.join(repoRoot, "docs", "demo", "express-happy-path");
...
function buildOpenApiSnippet(openApiContent) {
  ...
  const createUserOperation = openApi.paths?.["/api/v1/users"]?.post;
