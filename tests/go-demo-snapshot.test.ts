...
describe("Go demo snapshots", () => {
  it("matches the checked-in happy path output", async () => {
    const sandboxRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "brunogen-go-demo-"),
    );

    try {
      const fixtureRoot = fixturePath("gin");
      const workingRoot = path.join(sandboxRoot, "gin");
      ...
      const actualOpenApiSnippet = normalizeSnapshot(
        buildOpenApiSnippet(openApiContent),
      );
      const expectedOpenApiSnippet = await readDemoSnapshot(
        "openapi-snippet.yaml",
      );
      expect(actualOpenApiSnippet).toBe(expectedOpenApiSnippet);

      const actualBrunoRequest = normalizeSnapshot(
        await fs.readFile(
          path.join(generatedRoot, "bruno", "api", "createuser.bru"),
          "utf8",
        ),
      );
      const expectedBrunoRequest = await readDemoSnapshot(
        "bruno",
        "api",
        "createuser.bru",
      );
...
async function readDemoSnapshot(...segments: string[]): Promise<string> {
  const content = await fs.readFile(
    repoPath("docs", "demo", "go-happy-path", ...segments),
    "utf8",
  );
