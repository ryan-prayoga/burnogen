...
describe("Laravel adapter", () => {
  ...
  it("infers simple and cursor paginator resource envelopes", async () => {
    ...
    const collectionJsonWrapped = artifacts.normalized.endpoints.find(
      (endpoint) =>
        endpoint.path === "/api/projects/collection-json-wrapped" &&
        endpoint.method === "get",
    );
    const collectionJsonWrappedAccepted = collectionJsonWrapped?.responses.find(
      (response) => response.statusCode === "202",
    );
    expect(collectionJsonWrapped?.parameters).toContainEqual(expect.objectContaining({
      name: "page",
      in: "query",
    }));
    expect(collectionJsonWrappedAccepted?.schema?.properties?.data).toBeUndefined();
    expect(collectionJsonWrappedAccepted?.schema?.properties?.transformed?.type).toBe("array");
    expect(
      collectionJsonWrappedAccepted?.schema?.properties?.transformed?.items?.properties?.position?.type,
    ).toBe("integer");
    expect(
      collectionJsonWrappedAccepted?.schema?.properties?.transformed?.items?.properties?.identifier?.type,
    ).toBe("integer");
    expect(
      collectionJsonWrappedAccepted?.schema?.properties?.transformed?.items?.properties?.owner?.type,
    ).toBe("string");
    expect(
      collectionJsonWrappedAccepted?.schema?.properties?.transformed?.items?.properties?.label?.type,
    ).toBe("string");
    expect(
      collectionJsonWrappedAccepted?.schema?.properties?.meta?.properties?.per_page?.type,
    ).toBe("integer");
    expect(collectionJsonWrappedAccepted?.example).toEqual({
      transformed: [
        {
          position: 0,
          identifier: 1,
    ...
    const collectionResponseHeaders = artifacts.normalized.endpoints.find(
      (endpoint) =>
        endpoint.path === "/api/projects/collection-response-headers" &&
        endpoint.method === "get",
    );
    const collectionResponseHeadersMulti = collectionResponseHeaders?.responses.find(
      (response) => response.statusCode === "207",
    );
    expect(collectionResponseHeaders?.parameters).toContainEqual(expect.objectContaining({
      name: "page",
      in: "query",
    }));
    expect(collectionResponseHeadersMulti?.schema?.properties?.data?.type).toBe("array");
    expect(
      collectionResponseHeadersMulti?.schema?.properties?.data?.items?.properties?.id?.type,
    ).toBe("integer");
    expect(
      collectionResponseHeadersMulti?.schema?.properties?.data?.items?.properties?.name?.type,
    ).toBe("string");
    expect(
      collectionResponseHeadersMulti?.schema?.properties?.data?.items?.properties?.owner_email?.type,
    ).toBe("string");
    expect(
      collectionResponseHeadersMulti?.schema?.properties?.meta?.properties?.per_page?.type,
    ).toBe("integer");
    expect(collectionResponseHeadersMulti?.example).toEqual({
      data: [
        {
          id: 1,
          name: "Jane Doe",
