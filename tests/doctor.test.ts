...
describe("Doctor command", () => {
  ...
  it("resolves doctor paths from explicit config location", async () => {
    const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "brunogen-doctor-"));

    try {
      const workspaceRoot = path.join(sandbox, "workspace");
      const configRoot = path.join(workspaceRoot, "configs");
      const projectRoot = path.join(workspaceRoot, "apps", "express-custom-auth");
      await fs.mkdir(configRoot, { recursive: true });
      await fs.mkdir(path.dirname(projectRoot), { recursive: true });
      await fs.cp(fixturePath("express-custom-auth"), projectRoot, { recursive: true });
      ...
      const result = await runDoctor(workspaceRoot, loaded.config, loaded.configPath);

      expect(result.lines).toContain(`config: ${configPath}`);
      expect(result.lines).toContain(`project root: ${path.resolve(configRoot, config.inputRoot)}`);
      expect(result.lines).toContain(`openapi output: ${path.resolve(configRoot, config.output.openapiFile)}`);
      expect(result.lines).toContain(`bruno output: ${path.resolve(configRoot, config.output.brunoDir)}`);
